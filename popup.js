...
function refreshQQStatus() {
  ...
  requestStatus('qq.status', function (error, response) {
    ...
    if (status.loggedIn && status.playbackKeyReady) {
      var authHint = status.cookieCount > 1 ? 'Cookie 播放授权已检测到' : '页面播放授权已检测到';
      setStatus('qq', 'ok', 'QQ 音乐网页登录态已检测到', (status.nickname || status.userId || '已登录') + tabInfo + ' · ' + authHint + ' · 可尝试 QQ-X 搜索和播放' + cookieNames);
