...
const PORT = process.env.MINERADIO_BRIDGE_PORT || process.env.PORT || 37891;
const HOST = '127.0.0.1';
const WEB_APP_URL = process.env.MINERADIO_WEB_URL || 'https://mrchenyh.github.io/Mineradio-Web/';
const BRIDGE_ORIGIN = `http://${HOST}:${PORT}`;
const TOKEN_FILE = process.env.MINERADIO_BRIDGE_TOKEN_FILE || path.join(__dirname, '.mineradio-bridge-token');
const ALLOWED_WEB_ORIGINS = new Set((process.env.MINERADIO_WEB_ORIGINS || 'https://mrchenyh.github.io,http://127.0.0.1:4173,http://localhost:4173,http://127.0.0.1:5173,http://localhost:5173')
  .split(',')
...
const DISABLED_WEB_UPDATE = { ok: false, disabled: true, error: 'WEB_BRIDGE_UPDATE_DISABLED', message: 'Mineradio Web is updated by GitHub Pages. Bridge updates are distributed separately.' };
...
function readOrCreateBridgeToken() {
  const envToken = String(process.env.MINERADIO_BRIDGE_TOKEN || '').trim();
  ...
  try {
    ...
    console.warn('[Bridge] token persist failed:', e.message);
  }
  return token;
}
const BRIDGE_TOKEN = readOrCreateBridgeToken();
...
function isLocalDevOrigin(origin) {
  try {
    ...
    return (u.hostname === '127.0.0.1' || u.hostname === 'localhost') && /^\d+$/.test(u.port || '');
  } catch (_) { return false; }
}
function isAllowedWebOrigin(origin) {
  const normalized = normalizeOrigin(origin);
  if (!normalized) return true;
  return ALLOWED_WEB_ORIGINS.has(normalized) || isLocalDevOrigin(normalized);
}
function corsHeadersFor(req) {
  const origin = normalizeOrigin(req.headers.origin || '');
  const allowOrigin = isAllowedWebOrigin(origin) ? (origin || '*') : 'null';
  return {
    'Access-Control-Allow-Origin': allowOrigin,
    'Access-Control-Allow-Methods': 'GET,POST,OPTIONS',
    'Access-Control-Allow-Headers': 'Content-Type, X-Mineradio-Token, Range',
    'Access-Control-Allow-Private-Network': 'true',
    'Access-Control-Max-Age': '86400',
    'Vary': 'Origin, Access-Control-Request-Private-Network',
  };
}
function applyCors(res, req, extra) {
  const headers = Object.assign({}, corsHeadersFor(req || { headers: {} }), extra || {});
  Object.keys(headers).forEach(key => res.setHeader(key, headers[key]));
}
function readRequestToken(req, url) {
  return String(req.headers['x-mineradio-token'] || (url && url.searchParams.get('token')) || '').trim();
}
function isAuthorizedRequest(req, url) {
  return readRequestToken(req, url) === BRIDGE_TOKEN;
}
function sendBridgeJSON(req, res, data, status) {
  applyCors(res, req, {
...
function isBridgeStaticPath(pathname) {
  return pathname === '/' || pathname === '/favicon.ico' || pathname.startsWith('/assets/') || pathname.startsWith('/vendor/');
}
function isPublicBridgePath(pathname) {
  return pathname === '/health' || pathname === '/pair' || isBridgeStaticPath(pathname);
}
function bridgePairUrl(returnTo) {
  let target = WEB_APP_URL;
  try {
    if (returnTo) {
      const candidate = new URL(returnTo);
      if (isAllowedWebOrigin(candidate.origin)) target = candidate.toString();
    }
  } catch (_) {}
  const u = new URL(target);
  u.searchParams.set('bridge', BRIDGE_ORIGIN);
  u.searchParams.set('token', BRIDGE_TOKEN);
  return u.toString();
}
function isAllowedProxyTarget(targetUrl, kind) {
  try {
    ...
    if (kind === 'audio') {
      return host.endsWith('.music.126.net')
        ...
        || host === 'mqqmusic.tc.qq.com'
        || /^[a-z0-9-]*qqmusic\.tc\.qq\.com$/.test(host)
        || host === 'music.126.net'
...
function sendJSON(res, data, status) {
  const req = res.__mineradioReq || { headers: {} };
  sendBridgeJSON(req, res, data, status);
}
...
function coverProxyHeadersFor(coverUrl) {
  ...
  try {
    ...
    if (host.includes('qq.com') || host.includes('qpic.cn') || host.includes('qlogo.cn') || host.includes('gtimg.cn')) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/pair') {
    applyCors(res, req, { Location: bridgePairUrl(url.searchParams.get('return') || url.searchParams.get('to')) });
    res.writeHead(302);
    res.end('Pair Mineradio Web');
    return;
  }
  if (!isPublicBridgePath(pn) && !isAuthorizedRequest(req, url)) {
    sendBridgeJSON(req, res, { ok: false, error: 'UNAUTHORIZED_BRIDGE_REQUEST' }, 401);
    return;
  }
  if (pn === '/auth') {
    sendBridgeJSON(req, res, {
  ...
  if (pn === '/status') {
    const netease = await getLoginInfo().catch(err => ({ loggedIn: false, error: err.message }));
    const qq = await getQQLoginInfo().catch(err => ({ provider: 'qq', loggedIn: false, error: err.message }));
    sendBridgeJSON(req, res, {
      ok: true,
      bridge: {
        authenticated: true,
      ...
      platforms: {
        netease: {
          loggedIn: !!netease.loggedIn,
          nickname: netease.nickname || '',
          userId: netease.userId || '',
          vipLevel: netease.vipLevel || 'none',
        },
        qq: {
          loggedIn: !!qq.loggedIn,
          nickname: qq.nickname || '',
          userId: qq.userId || qq.uin || '',
  ...
  if (pn.startsWith('/api/update/')) {
    sendBridgeJSON(req, res, DISABLED_WEB_UPDATE, 410);
    return;
  }
  ...
  if (pn === '/api/cover') {
    try {
      ...
      if (!coverUrl || !isAllowedProxyTarget(coverUrl, 'cover')) {
        applyCors(res, req);
        res.writeHead(400);
      ...
      Object.assign(hdr, corsHeadersFor(req));
      res.writeHead(resp.status, hdr);
      const reader = resp.body.getReader();
      while (true) { const c = await reader.read(); if (c.done) break; res.write(c.value); }
      res.end();
    } catch (err) { console.error('[Cover]', err); applyCors(res, req); res.writeHead(500); res.end(); }
    return;
  }
  ...
  if (pn === '/api/audio') {
    try {
      const audioUrl = url.searchParams.get('url');
      if (!audioUrl) { applyCors(res, req); res.writeHead(400); res.end('Missing url'); return; }
      if (!isAllowedProxyTarget(audioUrl, 'audio')) { applyCors(res, req); res.writeHead(400); res.end('Invalid audio url'); return; }
      ...
      Object.assign(out, corsHeadersFor(req));
      res.writeHead(up.status, out);
      const reader = up.body.getReader();
      while (true) { const c = await reader.read(); if (c.done) break; res.write(c.value); }
      res.end();
    } catch (err) { console.error('[Audio]', err); applyCors(res, req); res.writeHead(500); res.end(); }
    return;
  }
...
server.listen(PORT, HOST, () => {
  console.log('======================================================');
  console.log(' Mineradio Bridge  ->  ' + BRIDGE_ORIGIN);
  console.log(' Pair URL: ' + BRIDGE_ORIGIN + '/pair');
  console.log(' Login: ' + (userCookie ? 'Netease cookie loaded' : 'Netease not logged in'));
  console.log('======================================================');
});
